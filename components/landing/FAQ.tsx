      ...
      <div className="flex flex-col items-center justify-center px-4 py-10 max-w-7xl w-full gap-12">
        ...
        <div className="w-full max-w-4xl">
          {faqData.map((item) => (
            <div
              key={item.id}
              className="mb-4 bg-card border border-border rounded-lg overflow-hidden transition-colors"
            >
              <button
                type="button"
                onClick={() => toggleItem(item.id)}
                className="w-full flex items-center justify-between p-5 text-left hover:bg-accent-subtle"
              >
